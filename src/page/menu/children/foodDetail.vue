    ...
    <div class="food-wrapper">
      <div class="detail">
        <div v-if="chooseType === 'food'" class="info">
          <div class="cover">
            <img width="100%" :src="food.bigImageUrl">
          </div>
          <dl class="content">
            <dt class="title">{{food.spuName}}</dt>
            <dd class="sale-num"><span class="num">月售{{food.saleVolumeDecoded}}</span>赞{{food.praiseNumDecoded}}</dd>
            <dd class="label" v-if="food.productLabelPictureList">
        ...
        <div v-if="chooseType === 'specification'" class="specification">
          ...
          <div class="content">
            <dl v-for="(spuItem,index) in food.spuAttrList" :key="index">
              <dt class="dt-item">{{spuItem.spuAttrName}}：</dt>
              <dd class="dd-item" v-for="item in spuItem.spuAttrValueList" :key="item.attrId"
               :class="{'active': attrs.includes(item.attrId)}" @click="selectSpecification(item.attrId,item.attrValue,index)">
                {{item.attrValue}}
              </dd>
          </dl>
        </div>
        ...
        <div class="price-unit">
           ...
           <div class="cartcontrol-wrapper">
              ...
              <cart-control v-show='foodNum' @showSpecification='showSpecification()'  :food='food' :shopId='shopId' :attrs='attrs' :attrValues='attrValues' :chooseType='chooseType' ref="cartcontrol"></cart-control>
           </div>
        </div>
      </div>
